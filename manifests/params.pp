class splunk::params {
  ...
  $splunk_ver          = $::splunk_ver #'4.3.1-119532'
  $installer           = $deploy ? {
                           'server' => $::architecture ? {
                             'i386' => $::operatingsystem ? {
                               ...
                               /(?i)(debian|ubuntu)/ => "splunk-${splunk_ver}-linux-2.6-intel.deb",
                             },
                             /(?i)(x86_64|amd64)/ => $::operatingsystem ? {
                               'windows'           => "splunk-${splunk_ver}-x64-release.msi",
                               /(?i)(centos|redhat)/ => "splunk-${splunk_ver}-linux-2.6-x86_64.rpm",
                               /(?i)(debian|ubuntu)/ => "splunk-${splunk_ver}-linux-2.6-amd64.deb",
                             },
                           },
